...
- 2026-05-04 v2：寫作哲學 + 軸心 reference 落地。新增 `references/writing-philosophy.md`（軸心 show, don't tell + 5 延伸 + 三篇範例引用，描述性 reference 不 gate）。Stage 1 框架庫加 (a) Narrative-arc / (b) Thesis-argumentation 兩型 process-driven 框架（不刪舊的 16+1）。Stage 5 加字句層 Lint hard rule（採範例 3 的 13 條裡的條 7/9/13：字短料多 / 刪雜質 / 一篇一個重點），新增 `lint_passed: true` frontmatter 欄位 + Gate 5→6 checklist 項目。`threads-angle-gate` skill 不動（避免訪談紀律 overload）。Spec：`docs/superpowers/specs/2026-05-04-threads-write-post-v2-design.md`。
