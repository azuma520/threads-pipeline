...
**日期**：2026-04-14
**狀態**：已整合 codex-architect + tech-spec-reviewer 審查意見，待使用者最終批准
...
**Subprocess 呼叫對齊既有模式**：`planner._call_claude()` 沿用 `analyzer.py` 的 `subprocess.run` 風格，並複用 `advisor.review_draft`（`advisor.py:234-246`）已加入的 Windows `shell=use_shell` 處理（`platform.system() == "Windows"`），避免回退 7ee59e0 修過的 Windows 相容性坑。

**既有 `advisor.review_draft` 的相容修改**：`advisor.py:211` 的 `plan_content[:2000]` 放寬到 `[:4000]`，讓 review 能吃到完整骨架。這是本設計對既有 code 的唯一侵入性改動。
...
題目 ─────────────→ │  claude -p --model haiku ──→ JSON
                    ...
                    └────────────────────────────────────────┘
                        ...
                        人工選 / --framework / --auto
                                    ↓
                    ┌─ 單一框架章節（從 md 切出）
[Stage 2]           ├─ SQLite top N 貼文 full_text（N=--style-posts，預設 3）
...
advisor plan "題目" --style-posts 5           # 風格範本條數，預設 3（早期數據少）
advisor plan "題目" --model haiku             # 覆寫 Stage 2 模型，預設 sonnet（亦可於 config.yaml 設 advisor.plan.stage2_model）
advisor plan "題目" --suggest-only --json     # 只回建議、不生檔；吐 JSON 到 stdout
# schema：{"suggestions": [{"framework": 11, "name": "逆襲引流", "reason": "...", "rank": 1}, ...]}
...
**輸出解析**：`json.loads(stdout)`；失敗 → **exit code 3，不寫檔，stderr 顯示解析失敗訊息**。使用者可重試或改用 `--framework N` 手動指定。**不做沉默 fallback**——讓使用者知道 LLM 失敗，而非永遠拿到被預設框架覆蓋的錯誤結果。
...
**核心設計**：
1. 產出是**骨架而非成稿**。每條貼文給內容方向，使用者自己填——維持使用者創作主體性，也符合「分享自己」的內容哲學。
2. **章節順序刻意前置「目標受眾」**：`advisor review` 的第 5 維度「受眾匹配」會參考 plan 裡的受眾設定（`advisor.py` REVIEW_PROMPT_TEMPLATE）；即使未來 review 截斷上限又變動，關鍵資訊也不會掉。
3. **總長 ≤3500 字**：搭配 `review` 端放寬到 4000（含少量緩衝），確保 review 不會截斷尾段互動設計與風險提示。

### Token 預算估計

- Stage 1：frameworks md（~2k 字） + 題目 ≈ 2.5k input tokens
- Stage 2：單一框架 + top 3 `full_text`（~1.5k 字） + 題目 ≈ 2k input / 1.5k output
...
| top posts 不足 3 篇 | 有幾篇用幾篇，stderr 警告；**0 篇 → prompt 中整個「## 風格範本」段落移除**（不留空標題） | 0 |
| `claude -p` 超時（60s） | retry 1 次，再失敗回報 | 3 |
| Stage 1 JSON 解析失敗 | **不寫檔，stderr 提示改用 `--framework N` 重試** | 3 |
...
- Stage 1 JSON 解析失敗 → exit code 3、不寫檔、stderr 訊息正確
...
### 已決議事項（原未決，兩位 reviewer 已給意見）

- [x] **Stage 2 預設 sonnet**：是。`config.yaml` 新增 `advisor.plan.stage2_model`，CLI `--model` 為最高優先；理由：haiku 生中文創意骨架易 AI 味爆表、觸發 eval rubric 第 5 條
- [x] **`--style-posts` 預設 3**：早期 top posts 數少且品質不齊，5 篇會稀釋訊號；資料成熟後再調回 5
- [x] **`list-frameworks` 獨立子指令**：是。保持 agent 介面一致（可 `--json`），避免 `plan` 指令語意膨脹

### 兩位 reviewer 補挖出、值得補註的風險

- **Prompt injection**：題目直接塞進 Stage 1/2 prompt。個人工具風險低，本版**不加長度上限**；若未來共享使用再加。
- **`full_text` 送 Anthropic**：Stage 2 把使用者 top N 貼文完整文字送到 Anthropic API。因為這些是已公開貼文，隱私上可接受，但此行為本文件明記。
- **OneDrive 路徑 + 中文 slug NFC/NFD**：未處理。若之後 git 出現 slug 重複/消失再補 `unicodedata.normalize('NFC', ...)`。
- **Layer 3 rubric「AI 味」第 5 條主觀**：目前沒給 judge subagent 負面樣本清單。Phase 1 先跑看看離散度，若 judge 分數不穩再補樣本。
...
- [x] **codex-architect 審查完成（2026-04-14）**：整體 4.5/5，同意兩階段設計、模組拆分、Stage 2 sonnet 取捨；必改點已整合進本文件
- [x] **tech-spec-reviewer 審查完成（2026-04-14）**：挖出兩項 blocking（plan.md 2000 字截斷、受眾欄位缺失），已修；fallback 到 framework 15 被否決，改為 exit 3 + 不寫檔
- [ ] 使用者最終審查（待進行）

### 審查帶來的主要修改摘要

1. **plan.md ≤3500 字** + `review` 端截斷上限放寬 4000（方案 C，避免骨架被壓扁）
2. **新增「## 目標受眾」章節**（前置於骨架，給 `review` 第 5 維度用）
3. **Stage 1 JSON 解析失敗 → exit 3，不沉默 fallback**
4. **`--suggest-only --json` schema 明定**：`{suggestions: [{framework, name, reason, rank}]}`
5. **Subprocess 呼叫對齊 `analyzer.py` + 複用 Windows `shell=True` 處理**
6. **`--style-posts` 預設 3**、**Stage 2 預設 sonnet 並可用 config.yaml 覆寫**、**`list-frameworks` 保留獨立**
