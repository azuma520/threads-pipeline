...
- `threads_pipeline/templates/plan_output.md.j2` — Jinja2 模板（Stage 2 輸出給 LLM 當格式參考，最終 plan.md 直接是 LLM 原始輸出）
...
def list_frameworks(md: str) -> list[dict]:
    ...
    out = []
    for m in _FRAMEWORK_ROW.finditer(md):
        fw_id = int(m.group(1))
        out.append({
            "id": fw_id,
...
Reference: `threads_pipeline/analyzer.py` for existing `claude -p` subprocess pattern, and `threads_pipeline/advisor.py:234-246` for Windows `shell=True` handling.
...
def _call_claude(prompt: str, model: str) -> str:
    """Run `claude -p --model <model>`, return stdout.

    Aligned with analyzer.py's subprocess pattern and advisor.review_draft's
    Windows shell handling (see advisor.py:234-246 and commit 7ee59e0).
    Retries once on timeout, raises PlannerError on any other failure.
    """
...
## Task 7: advisor.py — raise review truncation limit

This is a tiny prerequisite change to unblock Stage 2 骨架 not being chopped.

**Files:**
- Modify: `threads_pipeline/advisor.py:211`
- Test: `tests/test_advisor.py`

- [ ] **Step 1: Write failing test for the new limit**
...
def test_review_prompt_plan_truncation_is_4000():
    ...
    # 但 5000 個 P 全部不會出現在 prompt
    assert "P" * 4001 not in prompt
```

- [ ] **Step 2: Run, verify fail**

Run: `PYTHONUTF8=1 python -m pytest threads_pipeline/tests/test_advisor.py::test_review_prompt_plan_truncation_is_4000 -v`
Expected: FAIL — current limit is 2000
...
- [ ] **Step 5: Run full advisor test suite to ensure no regression**
...
git commit -m "fix(advisor): raise review plan truncation 2000 to 4000

So advisor plan's Stage 2 output (<=3500 chars) is not chopped mid-section."
```

---

## Task 8: config.yaml — advisor.plan.stage2_model
...
## Task 9: advisor.py — add `plan` subcommand
...
### 9a: CLI plumbing, non-interactive (`--auto` / `--framework` / `--suggest-only`)
...
Edit `threads_pipeline/advisor.py` — add near top-level constants (after existing `TEMPLATES_DIR`):

```python
FRAMEWORKS_MD_PATH = str(Path(__file__).parent.parent / "references" / "copywriting-frameworks.md")
DRAFTS_DIR = str(Path(__file__).parent.parent / "drafts")
```
...
```python
    ...
    plan_parser.add_argument("--overwrite", action="store_true", help="檔案已存在直接覆寫")
    plan_parser.add_argument("--no-overwrite", action="store_true", help="檔案已存在即報錯")
...
def _interactive_pick_framework(topic: str, frameworks_md: str) -> int | str | None:
    """暫時用 auto 行為；Task 9b 會替換為真正的互動 UI。"""
    from threads_pipeline import planner
...
### 9b: Interactive framework picker (`[1/2/3/a/q]`)
...
- [ ] **Step 1: Ensure DB exists (or warn)**

Run: `ls threads_pipeline/output/threads_insights.db 2>&1 || echo "先跑 pipeline 或走無風格範本"`

- [ ] **Step 2: Smoke test — list-frameworks**

Run from `桌面/`:
```bash
PYTHONUTF8=1 python -m threads_pipeline.advisor list-frameworks
```
Expected: 列出 16 個框架，每個有編號、名稱、公式、適用。

Run: `PYTHONUTF8=1 python -m threads_pipeline.advisor list-frameworks --json | python -c "import sys, json; data = json.load(sys.stdin); print(f'{len(data)} frameworks, ids: {[d[\"id\"] for d in data]}')"`
Expected: `16 frameworks, ids: [1, 2, ..., 16]`

- [ ] **Step 3: Smoke test — plan with forced framework**
...
```
Expected:
- stderr 顯示 `✓ plan.md 已寫入：...（框架 11）`
- stdout 顯示 plan.md 完整路徑
- 檔案 `drafts/我學-claude-code-一個月的心得.plan.md` 存在
- 內容包含「目標受眾」「骨架」「互動設計」章節
- 總長 ≤3500 字

驗證：
```bash
wc -m drafts/我學-claude-code-一個月的心得.plan.md
grep -c "## 目標受眾" drafts/我學-claude-code-一個月的心得.plan.md
```
Expected: 字數 <3500；受眾章節存在（count 1）。

- [ ] **Step 4: Smoke test — suggest-only JSON**

Run:
```bash
PYTHONUTF8=1 python -m threads_pipeline.advisor plan "給新手的 AI 工具入門指南" --suggest-only --json
```
Expected: stdout 吐 JSON `{"suggestions": [...]}`, 有 3 個 rank=1/2/3 項。

- [ ] **Step 5: Smoke test — review 能吃新 plan.md**
...
```
Expected: review 正常跑完，顯示六維度結果；stderr 不出現「找不到 plan」。

- [ ] **Step 6: Commit smoke results if any drafts were committed**

```bash
# 若 drafts/ 在 .gitignore 則無事可做
git status
```

---

## Task 11: Layer 3 eval — LLM-as-judge
...
- [ ] **Step 1: Create eval folder + golden topics**
...
JUDGE_RUBRIC = """你是 Threads 內容審查專家。請根據下列 5 個維度，為這份 plan.md 的品質打分（1-5 分）：
...
def _run_advisor_plan(topic: str, framework: int) -> str:
    """真實呼叫 advisor plan，回傳 plan.md 內容。"""
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        # 將 drafts 重導至臨時目錄 (透過環境變數不行，改讀回傳路徑)
        result = subprocess.run(
            ["python", "-m", "threads_pipeline.advisor", "plan", topic,
             "--framework", str(framework), "--auto", "--overwrite"],
            cwd=Path(__file__).parent.parent.parent.parent,  # 到 桌面/
            capture_output=True, text=True, encoding="utf-8",
            env={"PYTHONUTF8": "1"}, timeout=120,
        )
        if result.returncode != 0:
            raise RuntimeError(f"advisor plan failed: {result.stderr}")
        plan_path = result.stdout.strip().splitlines()[-1]
...
def _judge_with_subagent(plan_content: str, framework_info: str) -> dict:
    ...
    prompt = JUDGE_RUBRIC.format(framework_info=framework_info, plan_content=plan_content)
    import platform
    result = subprocess.run(
        ["codex", "exec", "-s", "read-only", "-"],
        input=prompt, capture_output=True, text=True, encoding="utf-8",
        timeout=120, shell=platform.system() == "Windows",
    )
    if result.returncode != 0:
...
def pytest_addoption(parser):
    parser.addoption("--run-llm-evals", action="store_true", default=False)
```
...
## Task 12: CLAUDE.md usage examples
...
1. **Spec coverage** — map spec sections to tasks:
   - §3 架構 (planner.py + advisor.py split) → Tasks 1-9
   - §4 CLI 介面 → Task 9 (non-interactive) + 9b (interactive)
   - §5 Prompt 設計 → Task 4
   - §6 錯誤處理 → Task 9 (all exit codes covered in tests)
   - §7 Layer 1/2 → Tasks 1-9 embed unit + integration tests
   - §7 Layer 3 → Task 11
   - §8 已決議事項 → Task 8 (config.yaml)
   - §9 落地計畫 8 項 → Tasks 1-12
