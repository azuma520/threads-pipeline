...
### 未來 roadmap（Level 1 後要做的事）

#### Ruff 納入 pyproject.toml

**現況**：2026-04-15 ruff 0.14.14 跑過本專案，發現 16 個警告：

| 代號 | 數量 | 性質 |
|---|---|---|
| F401 unused-import | 9 | 多在 `scripts/api_explorer.py` |
| E402 import not at top | 5 | `advisor.py` 等刻意把 CLI import 放下面 |
| E741 ambiguous var name | 1 | `analyzer.py:88` 用了 `l` |
| F541 empty f-string | 1 | 可自動修 |

**結論**：Level 1 不納入。理由：
- 16 個警告都輕、無功能影響
- 納入會變成 CI 噪音
- E402 多為刻意寫法，需人眼判斷保留或改位置

**納入時機**：Level 1 + E-輕完成、穩定運行一段時間後，獨立 PR 處理。

**納入步驟**：
```bash
# 1. 安全自動修（review diff）
ruff check --fix --unsafe-fixes . \
  --exclude='cli-anything-ref,docs,output'

# 2. 人工處理 E402（加 # noqa 或改位置）
# 3. 人工處理 E741（l → line）

# 4. 加 [tool.ruff] 到 pyproject.toml
#    line-length = 100
#    target-version = "py313"
#    extend-exclude = ["cli-anything-ref", "docs", "output"]

# 5. CI 加 ruff check 作為 pre-commit 或 GitHub Actions
```

#### E-重擴充項（Level 1 穩定後評估）

- 串文（thread chain）發文
- 從 `advisor plan` 結果直接發文
- 排程發文
- 發文模板
- 發文前自動跑 `advisor review`
- 發文後自動寫入 SQLite 紀錄

#### 加入 cli-anything Hub（遠期）

觸發條件：
- Level 1 + E-輕 穩定
- Advanced Access 核准（讓跨帳號搜尋功能可用）
- 決定要讓別人也能 `pip install threads-pipeline` 裝起來用

動作：
- 撰寫 `SKILL.md`（Agent 使用說明）
- 對 HKUDS/CLI-Anything 發 registry-only PR（CONTRIBUTING 路徑 B）
- 考慮發佈 PyPI 讓 install_cmd 變短
